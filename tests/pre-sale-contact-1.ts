...
describe("pre-sale-contact", () => {
  ...
  const buyer = Keypair.fromSecretKey(
    base58.decode(
      "2uR9EaUBtzVMKet9PrTq84TcPPYqocZZ9Q5F2ps1MoU7f9f452PrW6QmnpEf8kYwv7tEi92SrwrzZtbfK1mSiDkH"
    )
  );
  ...
  const startTime = Math.floor(new Date().getTime() / 1000) + 2 * 60;
  ...
  it("create presale account", async () => {
    try {
      ...
      const presaleArgs = {
        ...
        tokenPriceInUsdc: new anchor.BN(2 * 1000000),
      };
  ...
  //       endTime: new anchor.BN(endTime + 2 * 60),
  //     };
  ...
  //   try {
  //     let paymentDecimalValue = 1000000;
  //     let tokenAmount = new anchor.BN(2 * paymentDecimalValue);
