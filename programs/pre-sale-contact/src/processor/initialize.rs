...
pub fn initialize_handler(ctx: Context<Initialize>, args: InitializeArgs) -> Result<()> {
    ...
    presale_program_data.authority = ctx.accounts.authority.key();
...
pub struct Initialize<'info>{
    ...
    #[account(init, seeds = [PREFIX, PROGRAM_DATA], bump, payer = authority, space = 8 + PresaleProgramData::INIT_SPACE)]
